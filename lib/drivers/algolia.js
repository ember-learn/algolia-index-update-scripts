...
function write(indexName, records) {
  ...
  return index
    .addObjects(records)
    .then(() =>
      console.log(`ALGOLIA:: Successfully added ${records.length} records to ${indexName}`)
    )
    .catch((err) =>
      console.error(`ALGOLIA:: Error adding ${records.length} records to ${indexName}`, err)
    );
}
...
async function getPreviouslyIndexedVersions(projectName) {
  ...
  let hit = hits
    .filter((hit) => hit.name === projectName && hit.index_date_timestamp)
    .sort(function (a, b) {
